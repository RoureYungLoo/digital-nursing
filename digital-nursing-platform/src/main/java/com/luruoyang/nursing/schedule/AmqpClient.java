...
public class AmqpClient implements ApplicationRunner {
  ...
  public void start() throws Exception {
    ...
    for (int i = 0; i < huaweiProperties.getConnectionCount(); i++) {
      // 创建amqp连接
      ...
      // 创建Receiver连接。
      MessageConsumer consumer = newConsumer(session, connection, huaweiProperties.getIotda().getAccess().getApplication().getAmqps().getQueueName());
  ...
  private Connection getConnection() throws Exception {
    ...
    String userName = "accessKey=" + huaweiProperties.getIotda().getAccess().getApplication().getAmqps().getAccessKey();
    ...
    // 创建连接。
    return cf.createConnection(userName, huaweiProperties.getIotda().getAccess().getApplication().getAmqps().getAccessCode());
  }
  ...
  public String generateConnectUrl() {
    String uri = MessageFormat.format("{0}://{1}:{2}",
        (huaweiProperties.isUseSsl() ? "amqps" : "amqp"),
        huaweiProperties.getIotda().getAccess().getApplication().getEndpoint(),
        String.valueOf(huaweiProperties.getIotda().getAccess().getApplication().getAmqps().getPort()));
    Map<String, String> uriOptions = new HashMap<>();
    uriOptions.put("amqp.vhost", huaweiProperties.getVhost());
    uriOptions.put("amqp.idleTimeout", String.valueOf(huaweiProperties.getIdleTimeout()));
    uriOptions.put("amqp.saslMechanisms", huaweiProperties.getSaslMechanisms());

    Map<String, String> jmsOptions = new HashMap<>();
    jmsOptions.put("jms.prefetchPolicy.queuePrefetch", String.valueOf(huaweiProperties.getQueuePrefetch()));
    ...
    jmsOptions.put("failover.reconnectDelay", String.valueOf(huaweiProperties.getReconnectDelay()));
    jmsOptions.put("failover.maxReconnectDelay", String.valueOf(huaweiProperties.getMaxReconnectDelay()));
    if (huaweiProperties.getMaxReconnectAttempts() > 0) {
      jmsOptions.put("failover.maxReconnectAttempts", String.valueOf(huaweiProperties.getMaxReconnectAttempts()));
    }
    if (huaweiProperties.getExtendedOptions() != null) {
      for (Map.Entry<String, String> option : huaweiProperties.getExtendedOptions().entrySet()) {
  ...
  private void processMessage(Message message) {
    ...
    try {
      ...
      if (StringUtils.isNotEmpty(contentStr)) {
        deviceDataService.batchInsertDeviceData(contentStr);
      } else {
        log.error("contentStr is empty, no message received from huawei iot platform");
      }
